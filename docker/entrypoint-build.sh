...
load_app_env() {
  ...
  if [ -z "$EXPO_PUBLIC_API_URL" ]; then
    echo "❌ EXPO_PUBLIC_API_URL não definida em app/.env"
...
CONFIG_HASH=$(
  cat /app/app/app.json \
      /app/app/app.config.js \
      /app/app/babel.config.js \
      /app/app/.env \
...
APK_PATH=$(find /app/app/android/app/build/outputs/apk -name "*.apk" -type f | head -1)
